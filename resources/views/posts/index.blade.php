...
@section('content')
    ...
    @foreach ($posts as $post)
        @if ($post->post_id != NULL )
            @continue
        @endif
        <p style="display:inline">{{ $post->id }}. 名前:{{ $post->name }}  投稿時間[{{ $post->created_at }}] ID:{{ $post->fixed_id}} </p>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="card-title">
                            <p style="display:inline; padding-right:25px">{{ $micropost->id }}. 名前:{{ $micropost->name }}  投稿時間[{{ $micropost->created_at }}] ID:{{ $micropost->fixed_id}} </p>
                            @component('components.btn-del')
                                @slot('action')
                                    {{'delete/'.$micropost->id}}
                                @endslot
